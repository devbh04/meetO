...
class MeetingProcessingPipeline:
    ...
    def process(self, recording_path: str, chat_messages: list[dict] | None = None, metadata: dict | None = None) -> dict:
        ...
        try:
            ...
            try:
                import asyncio
                from .db import save_meeting_to_db
                meeting_id = self.config.base_dir.name
                asyncio.run(save_meeting_to_db(meeting_id, str(self.config.base_dir), metadata))
            except Exception as db_err:
                print(f"[Pipeline] DB Save Error: {db_err}")
